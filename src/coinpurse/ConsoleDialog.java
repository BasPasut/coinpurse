...
public class ConsoleDialog {
	...
	private Purse purse;
	private MoneyFactory build = MoneyFactory.getInstance();
	...
	public void run() {
		...
		System.out.println("Goodbye. The purse still has " + purse.getBalance() + " " + currencies);
	}

	/**
	 * Ask the user how many coins or banknotes to deposit into purse, then
	 * deposit them. Show result of success or failure.
	 */
	public void depositDialog() {
		...
		while (scanline.hasNextDouble()) {
			...
			try {
				valuable = build.createMoney(value);
				System.out.printf("Deposit %s... ", valuable.toString());
			} catch (IllegalArgumentException ex) {
				System.out.println("Sorry, this value not valid for this currency.");
				continue;
			}
